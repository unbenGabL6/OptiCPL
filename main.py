...
from OptiCPL_refactored.optical_model import load_optical_model
...
def raw_pipeline_mode(config: Config):
    ...
    output_dict = process_raw_data(
        ...
        optical_model_path=config.raw_pipeline_optical_model_path,
...
Examples:
  ...
  python main.py train --config my_config.py
    ...
    parser.add_argument(
        "--optical-model",
        type=str,
        help="Path to optical model for Optical feature generation in raw-pipeline mode"
    )
    ...
    if args.optical_model:
        config.raw_pipeline_optical_model_path = args.optical_model
    if args.img_model:
