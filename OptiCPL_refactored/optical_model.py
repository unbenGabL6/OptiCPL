"""
Optical Feature Extractor
Handles loading and inference of the optical model for generating Optical features from spectral data
"""
...
class OpticalFeatureExtractor(nn.Module):
    """
    Multi-Input Feature Extractor for processing 5 types of spectral data
    Input: PL(40), EX_sample(200), CD_sample(200), CD_template(200), EX_template(200)
    Output: 32-dim optical features
    """
    ...
    def __init__(self, dims, latent_dim=32):
        ...
        # Independent decoders (for training reconstruction)
    ...
    def encode(self, pl, ex_sample, cd_sample, cd_template, ex_template):
        """Encode inputs to feature space (inference only)"""
        pl_feat = self.pl_encoder(pl)
        ex_sample_feat = self.ex_sample_encoder(ex_sample)
        cd_sample_feat = self.cd_sample_encoder(cd_sample)
        cd_template_feat = self.cd_template_encoder(cd_template)
...
def load_optical_model(checkpoint_path: str, device: str = 'cpu') -> OpticalFeatureExtractor:
    """
    Load optical model from checkpoint
    ...
    Returns:
        Loaded optical model in eval mode
    """
    # Model dimensions
    ...
    # Initialize model
    model = OpticalFeatureExtractor(dims, latent_dim)
...
def extract_optical_features(optical_model: OpticalFeatureExtractor, df: pd.DataFrame, device: str = 'cpu',
                             batch_size: int = 32) -> np.ndarray:
    """
    Extract Optical features using optical model

    Args:
        optical_model: Loaded optical model
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            features = optical_model.encode(batch_pl, batch_ex, batch_cd_s, batch_cd_t, batch_ex_t)
